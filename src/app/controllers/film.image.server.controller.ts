...
const getImage = async (req: Request, res: Response): Promise<void> => {
    try{
        ...
        const fileName = await images.getFilmImage(id);
        if (fileName === null) {
            Logger.error("no film with specified id, or film has no image");
...
const setImage = async (req: Request, res: Response): Promise<void> => {
    try{
        ...
        const isFilm = await images.checkFilmExists(id);
        if (!isFilm) {
            Logger.error(`no such film with id ${id} or file does not exist in database`);
            res.statusMessage = "Not Found";
            res.status(404).send();
            return;
        }
        if (directorId[0].director_id !== userId[0].id) {
            Logger.error("only the director of a film can change the film image or cannot edit a film that has a review placed");
            res.statusMessage = "Forbidden";
            res.status(403).send();
            return;
        } else {
            // delete existing film image
...
//         }
//
//         const isFilm = await images.checkFilmIdExists(userId[0].id);
//         if (directorId[0].director_id !== userId[0].id) {
//             Logger.error("only the director of a film can change the film image");
//             res.statusMessage = "Forbidden";
//             res.status(403).send();
//             return;
//         } else if (!isFilm) {
